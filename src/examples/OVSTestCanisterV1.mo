...
shared ({ caller = _owner }) persistent actor class OVSTestCanisterV1(_args: ?{
  collector: ?Principal;
  period: ?Nat;
}) = this {
  ...
  // OVS period override - default is 30 days, but we can override for testing
  var ovsPeriod : Nat = switch(_args) {
    case(?args) {
      switch(args.period) {
        case(?p) p;
        case(null) 60_000_000_000; // 1 minute for testing
      };
    };
    case(null) 60_000_000_000; // 1 minute for testing
  };
  ...
  private func handleICRC85Share(events: [(Text, ICRC85Map)]) : () {
    D.print("OVSTestCanisterV1: handleICRC85Share called with " # debug_show(events));
    cycleShareCount += 1;
    lastCycleShareTime := Int.abs(Time.now());
    
    for ((namespace, _map) in events.vals()) {
      D.print("OVSTestCanisterV1: ICRC85 share for namespace: " # namespace);
    };
  };

  // Sync handler: increment counter
  private func handleIncSync<system>(id: TT.ActionId, action: TT.Action) : TT.ActionId {
    D.print("OVSTestCanisterV1: handleIncSync " # debug_show(action));
  ...
  private func handleNoopSync<system>(id: TT.ActionId, action: TT.Action) : TT.ActionId {
    D.print("OVSTestCanisterV1: handleNoopSync");
    lastExecutedNamespace := action.actionType;
    lastExecutedParams := action.params;
    id;
  };
  ...
  private func handleDefaultSync<system>(id: TT.ActionId, action: TT.Action) : TT.ActionId {
    D.print("OVSTestCanisterV1: handleDefaultSync for " # action.actionType);
    lastExecutedNamespace := action.actionType;
    lastExecutedParams := action.params;
    id;
  };
  ...
  private func handleIncAsync<system>(id: TT.ActionId, action: TT.Action) : async* Star.Star<TT.ActionId, TT.Error> {
    D.print("OVSTestCanisterV1: handleIncAsync " # debug_show(action));
    let amt : ?Nat = from_candid(action.params);
    switch(amt) {
      case(?val) { counter += val; };
      case(null) { counter += 1; };
  ...
  private func reportExecution(execInfo: TT.ExecutionReport) : Bool {
    D.print("OVSTestCanisterV1: reportExecution " # debug_show(execInfo.action.1.actionType));
    executedActions += 1;
  ...
  private func reportError(errInfo: TT.ErrorReport) : ?Nat {
    D.print("OVSTestCanisterV1: reportError " # debug_show(errInfo));
    errorCount += 1;
    null;
  };
  ...
  transient let timerTool = TT.Init({
    ...
    pullEnvironment = ?(func() : TT.Environment {
      D.print("OVSTestCanisterV1: pulling environment");
      {      
        advanced = ?{
          icrc85 = ?{
    ...
    onInitialize = ?(func(tt: TT.TimerTool) : async* () {
      D.print("OVSTestCanisterV1: TimerTool initialized");
  ...
  D.print("OVSTestCanisterV1: Actor initialized");
};

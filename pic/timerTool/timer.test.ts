...
describe("test timers", () => {
  ...
  beforeEach(async () => {
    // Create PocketIC instance without NNS - simpler setup for timer tests
  ...
  // Skip NNS-dependent test - requires NNS state which is incompatible with current PocketIC version
  it.skip(`can create an approval`, async () => {
...
// Skip: This test requires OVS enabled, but the test canister uses kill_switch=true to disable OVS.
// OVS/ICRC-85 functionality is tested in mixin.test.ts instead.
it.skip('cycle share is processed correctly', async () => {

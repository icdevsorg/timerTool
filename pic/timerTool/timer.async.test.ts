...
export const sub_WASM_PATH = resolve(__dirname, "../../.dfx/local/canisters/timer/timer.wasm");
...
const NNS_STATE_PATH = resolve(__dirname, "../nns_state/node-100/state");
...
const base64ToUInt8Array = (base64String: string): Uint8Array => {
  // Buffer.from uses a shared pool, so we need to copy to a standalone ArrayBuffer
  // to avoid issues with Ed25519KeyIdentity checking the underlying buffer size
...
describe("test timers", () => {
  ...
  beforeEach(async () => {
    // Create PocketIC instance without NNS - simpler and faster setup for timer tests
    // NNS state loading is slow and can cause compatibility issues
  ...
  // Skip this test - it requires NNS state which has compatibility issues with newer PocketIC
  // TODO: Re-enable when NNS state is regenerated or use a mock ledger
  it.skip(`async can create an approval`, async () => {
...
it('async timer restarts properly after upgrade', async () => {
  ...
  // Simulate Canister Upgrade with Enhanced Orthogonal Persistence support

...
shared (deployer) persistent actor class TimerTool<system>(args : TT.Args)  = this {


  transient let OneMinute = 60_000_000_000;
 ...
 D.print("in test canister " # debug_show(args));
  ...
  transient var canisterId_ : ?Principal = null;

  private func _getCanister() : Principal {
    switch(canisterId_){
      case(null){
  ...
  private func reportExecution(_execInfo: TT.ExecutionReport): Bool{
      return false;
  };
  ...
  transient let initManager = ClassPlusLib.ClassPlusInitializationManager<system>(deployer.caller, Principal.fromActor(this), true);
  ...
  transient let timerTool = TT.Init({
    ...
    pullEnvironment = ?(func() : TT.Environment {
      ...
      {      
        advanced = ?{
          icrc85 = ?{
            kill_switch = ?true;  // Disable OVS for testing
            handler = null;
            period = null;
            initialWait = null;  // Use default 7 day grace period
            asset = null;
            platform = null;
            tree = null;
            collector = null;
    ...
    onInitialize = ?(func (newClass: TT.TimerTool) : async* () {
      ...
      // No need to call initialize - it's now automatic via Init wrapper
    });
  ...
  private func handleTimer<system>(id: TT.ActionId, action: TT.Action) : TT.ActionId{
    ...
    switch(action.actionType){
      case("inc"){
        ...
        let ?amt = candidParsed else Runtime.trap("nat nat");
      ...
      case("trapfor10"){
        ...
        {
          D.print("trapping");
          Runtime.trap("unexpected!");
        };
      };
      case("delay.1.minute"){
        D.print("in delay.1.minute sync " # debug_show(action));
        if(action.retries == 0) Runtime.trap("unexpected");
  ...
  transient var first_trap = 0 : Int;
